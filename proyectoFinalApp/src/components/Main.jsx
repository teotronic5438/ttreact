...
//             <h3 className='fs-4 text'>Estudiante de Talento Tech: React - Comisión 25021</h3>
  ...
  return (
    <main>
      <Container className="my-5">
        ...
        {/* Tecnologías */}
        <h2 className="mb-4 text-center">Conocimientos que estoy aprendiendo</h2>
        <Row className="g-4">
          {tecnologias.map((tech) => (
            <Col key={tech.nombre} xs={12} sm={6} md={4} lg={3}>
              <RBCard className="h-100 text-center shadow-sm">
                <RBCard.Img
                  variant="top"
                  src={tech.imagen}
                  alt={tech.nombre}
                  style={{ height: "100px", objectFit: "contain", padding: "1em" }}
                />
                <RBCard.Body>
                  <RBCard.Title>{tech.nombre}</RBCard.Title>
                  <RBCard.Text>{tech.descripcion}</RBCard.Text>
                </RBCard.Body>
              </RBCard>
            </Col>
          ))}
